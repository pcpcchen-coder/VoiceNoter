...
| 9 | 死碼大掃除 | ✅ | 2026-07-03 | `<this>` | 刪除 `OpenAIOAuthManager.swift`、`OpenAISettingsView.swift`（D1/D2）；`SettingsView` 移除 OAuth section 與 auto-reauth Toggle；`AudioRecorder` 刪 `cancel()`/`currentDuration`（D4）；`print(` 於正式碼歸零（D5/S2，隨 OAuth 檔刪除自動達成）；`handleAPIError`（B6）隨檔刪除。DoD：無 `print(`、無 `ASWebAuthenticationSession`/`AuthenticationServices` import |
| 10 | `RecordingCoordinator` 獨立化與依賴注入 | ✅ | 2026-07-03 | `<this>` | `RecordingCoordinator` 移到 `Core/RecordingCoordinator.swift`，8 個相依全注入（`AudioRecording`/`Transcribing`/`NoteStoring`/`TextRewriting`/`TranscriptDelivering`/`GlossaryStore`/`SettingsStore`/`AppState`）；新增 `AudioRecording` 與 `TranscriptDelivering` 協定（`SystemTranscriptDeliverer`）；`VoiceNoteApp` 瘦身至 49 行純組裝；新增 `RecordingCoordinatorTests`（10 個流程測試）+ `MockAudioRecorder`/`MockDeliverer`/`MockNoteStore`。DoD：VoiceNoteApp <100 行且無業務邏輯 ✓。手動 smoke test 待使用者於 Mac 執行 |
...
| 2026-07-03 | 10 | 計畫原訂 `scheduleWarmup` 在 coordinator init；改為 init 純儲存相依、另設 `activate()`（掛熱鍵 + 暖機），由 `VoiceNoteApp` 呼叫 | 暖機若在 init 觸發會與單元測試斷言競態（mock warmup 改 state）；分離後測試建構 SUT 不觸發副作用，直接驅動 `handlePress/handleRelease`。已同步更新 `REFACTOR_PLAN.md` Step 10 |
| 2026-07-03 | 10 | `handlePress`/`handleRelease`/`runTranscription` 改 internal；新增 `waitForPendingWork()` 測試輔助等待轉錄 Task | 讓 `@MainActor` 測試能同步驅動流程並 await 非同步結果 |
| 2026-07-03 | 11 | **使用者可見行為變更**：「幫我整理」由「整檔覆蓋」改為「追加 `## AI 整理 (HH:mm)` 段落」；成功訊息改為灰字資訊而非紅字；游標貼原始文字、筆記存校稿版（B5 取捨） | 修 B3 資料遺失風險 + B1 訊息語意錯誤；已於前一則對話向使用者說明並取得「請完成 Step 11」確認 |
| 2026-07-03 | 11 | 新增 `NoteStoring.appendSection` + `NoteFormatter.section`（計畫未列，但整理段落需要） | 「幫我整理」需以自訂標題段落追加；比重用 `append`（會加時間戳標題）更符合規格 |
